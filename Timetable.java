...
//Class timetable for creating and maintaining the timetable
public class Timetable {
    ...
    //Initializes the timetable as per the design for a month
    ...
    //Display the timetable for a specified Day
    ...
    //Display the timetable for a specified grade
                    ...
                    for (Map.Entry<String, Session> sessionEntry : weekDay.getValue().entrySet()) {
                        ...
                        if (session.getSessionID() == sessionID) {
                            ...
                            if (vacancies > 0 && (session.getGrade() == grade || session.getGrade() == grade+1)){
                                ...
                                Bookings bookings = new Bookings(learnerID, learnerName, weekNumber, weekDay.getKey(), sessionID, bookingID, status, session.getGrade(), true, false, false);
                                return bookings;
                            }
                            else {
    ...
    //Updates the sessions in the timetable
    ...
    //Add rating and review to coach on attending the sessions
    public void addCoachRating(int sessionID, String review, int rating, String weekNumber) {
        for (Map.Entry<String, Map<Integer, Map<String, Session>>> week : timetable.entrySet()) {
            if (week.getKey().equals("Week " + weekNumber)) {
                System.out.println("Week: " + week.getKey());
                for (Map.Entry<Integer, Map<String, Session>> weekDay : week.getValue().entrySet()) {
                    for (Map.Entry<String, Session> sessionEntry : weekDay.getValue().entrySet()) {
                        Session session = sessionEntry.getValue();
                        if (session.getSessionID() == sessionID) {
                            session.getCoach().setRating(rating);
                            session.getCoach().setReview(review);
                            return;
                        }
                    }
                }
    ...
    //To provide coach report
    public void coachReport(){
        System.out.println("\t\tCOACH REPORT");
        System.out.println("\t\t____________");
        for (int i =1;i<=coaches.size(); i++){
            System.out.println("________________________________");
            System.out.println("Coach Name: "+ coaches.get(i).getName());
            System.out.println("Coach Rating: "+ coaches.get(i).getAverageRating());
            System.out.println("Coach Remarks: "+ coaches.get(i).getRemarks(coaches.get(i).getAverageRating()));
            System.out.println("________________________________");
        }
    }
}

//Session class to keep track of the sessions and to update them in the timetable
class Session {
    private Coach coach;
    private int grade;
